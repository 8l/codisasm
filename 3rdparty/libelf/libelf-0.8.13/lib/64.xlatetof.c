...
_elf64_xltsize(const Elf_Data *src, unsigned dv, unsigned encode, int tof) {
    ...
    return (*op)(NULL, (unsigned char *)src->d_buf, src->d_size);
}
...
elf64_xlate(Elf_Data *dst, const Elf_Data *src, unsigned encode, int tof) {
    ...
    dsize = (*op)(NULL, (unsigned char *)src->d_buf, src->d_size);
    if (dsize == (size_t)-1) {
	return NULL;
    }
    if (dst->d_size < dsize) {
    ...
    if (dsize) {
	tmp = (*op)((unsigned char *)dst->d_buf, (unsigned char *)src->d_buf, src->d_size);
	if (tmp == (size_t)-1) {
	    return NULL;
	}
